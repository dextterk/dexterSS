...
function classifyIP(info, domain) {
  ...
  if (CHEAT_PROXY_ASN[asn]) {
    ...
    if (isCloudflare) {
      ...
      severity = "CRITICAL"
      reasons.push(`⚠ SERVIÇO DE PROXY DETECTADO — Hostinger BR (${asn}): provedor amplamente usado para hospedar proxies e cheats no Free Fire Brasil`)
    } else {
      severity = "HIGH"
  ...
  if (rdns) {
    ...
    if (rdns.match(/^srv\d+\.hstgr\.cloud$/)) {
      severity = "CRITICAL"
      reasons.push(`⚠ SERVIÇO DE PROXY DETECTADO — Hostinger VPS BR: servidor de proxy/cheat confirmado pelo padrão rDNS (${info.reverse})`)
    }
...
function buildHTML(findings, netEntries, cheatAppFindings, knownCheatFindings, ipsFindings, ipsMeta, replaceFindings, ghostAppFindings, proxyLoginFindings, filename) {
  ...
  // ── Detecção de manipulação de horário ────────────────────────────────────
  let timeAnomalies = []
  let timeBanner = ""

  let orderedEntries = netEntries
    .map(e => e.timeStamp ? { raw: e.timeStamp, date: new Date(e.timeStamp) } : null)
    .filter(e => e && !isNaN(e.date))

  let orderedTimestamps = orderedEntries.map(e => e.date)

  if (orderedTimestamps.length >= 5) {
    let now = new Date()

    // 0. Mudança de fuso horário (offset UTC diferente entre entradas)
    let tzOffsetRx = /([+-]\d{2}:\d{2}|Z)$/
    let seenOffsets = []
    for (let e of orderedEntries) {
      let m = e.raw.match(tzOffsetRx)
      if (!m) continue
      let tz = m[1] === "Z" ? "+00:00" : m[1]
      if (!seenOffsets.includes(tz)) seenOffsets.push(tz)
    }
    if (seenOffsets.length > 1) {
      let offsetList = seenOffsets.join(" → ")
      timeAnomalies.push({
        icon: "🌍",
        severity: "critical",
        title: "Fuso Horário Alterado",
        desc: `${seenOffsets.length} fusos horários diferentes detectados no relatório — dispositivo trocou de fuso durante a sessão`,
        detail: `Offsets UTC encontrados: ${offsetList}`
      })
    }

    // 1. Timestamps no futuro (> 5 min)
    let futureEntries = orderedTimestamps.filter(d => d > new Date(now.getTime() + 5 * 60000))
    if (futureEntries.length > 0) {
      let maxFuture = futureEntries.reduce((mx, d) => d > mx ? d : mx, futureEntries[0])
      timeAnomalies.push({
        icon: "⏩",
        severity: "critical",
        title: "Timestamp no Futuro",
        desc: `${futureEntries.length} registro(s) com data futura detectado(s) — possível manipulação de relógio`,
        detail: `Mais avançado: ${fmtDt(maxFuture)}`
      })
    }

    // 2. Saltos para o passado (relógio retroagido > 10 min)
    let backJumps = []
    for (let i = 1; i < orderedTimestamps.length; i++) {
      let diffMs = orderedTimestamps[i] - orderedTimestamps[i - 1]
      if (diffMs < -10 * 60000) {
        backJumps.push({
          from: orderedTimestamps[i - 1],
          to: orderedTimestamps[i],
          deltaMin: Math.abs(Math.floor(diffMs / 60000))
        })
      }
    }
    if (backJumps.length > 0) {
      let maxJump = backJumps.reduce((m, j) => j.deltaMin > m.deltaMin ? j : m, backJumps[0])
      timeAnomalies.push({
        icon: "⏪",
        severity: "critical",
        title: "Relógio Retroagido",
        desc: `${backJumps.length} salto(s) para o passado detectado(s) — provável manipulação de horário`,
        detail: `Maior salto: ${maxJump.deltaMin} min para trás — ${fmtDt(maxJump.from)} → ${fmtDt(maxJump.to)}`
      })
    }

    // 3. Saltos grandes para o futuro dentro de sessão curta (relógio adiantado)
    let totalSessionMin = firstTs && lastTs ? Math.floor((lastTs - firstTs) / 60000) : 0
    let bigForwardJumps = []
    for (let i = 1; i < orderedTimestamps.length; i++) {
      let diffMs = orderedTimestamps[i] - orderedTimestamps[i - 1]
      if (diffMs > 12 * 3600000) {
        bigForwardJumps.push({
          from: orderedTimestamps[i - 1],
          to: orderedTimestamps[i],
          deltaH: Math.floor(diffMs / 3600000)
        })
      }
    }
    if (bigForwardJumps.length > 0 && totalSessionMin < 24 * 60) {
      let maxJump = bigForwardJumps.reduce((m, j) => j.deltaH > m.deltaH ? j : m, bigForwardJumps[0])
      timeAnomalies.push({
        icon: "⏭",
        severity: "warning",
        title: "Salto de Horário Suspeito",
        desc: `${bigForwardJumps.length} salto(s) de mais de ${maxJump.deltaH}h detectado(s) dentro da sessão`,
        detail: `${fmtDt(maxJump.from)} → ${fmtDt(maxJump.to)}`
      })
    }
  }

  if (timeAnomalies.length > 0) {
    let rows = timeAnomalies.map(a => `
      <div class="time-row time-row-${a.severity}">
        <div class="time-row-icon">${a.icon}</div>
        <div class="time-row-body">
          <div class="time-row-title">${a.title}</div>
          <div class="time-row-desc">${a.desc}</div>
          <div class="time-row-detail">${a.detail}</div>
        </div>
      </div>`).join("")
    timeBanner = `
  <div class="time-banner">
    <div class="time-header">
      ...
      <div class="time-header-text">
        <div class="time-title">MANIPULAÇÃO DE HORÁRIO DETECTADA</div>
        <div class="time-sub">Inconsistências nos timestamps do relatório</div>
      </div>
      <span class="time-count">${timeAnomalies.length}</span>
    </div>
    <div class="time-rows">${rows}</div>
    <div class="time-hint">⚠ Anomalias nos timestamps indicam possível alteração manual do relógio do dispositivo para ocultar atividades</div>
  </div>`
  }
  // ──────────────────────────────────────────────────────────────────────────
...
<style>
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  /* TIME MANIPULATION BANNER */
  .time-banner {
    background:linear-gradient(135deg,#1a0800,#140500);
    border:1px solid #ff660066; border-radius:12px;
    padding:14px; margin-bottom:12px;
  }
  .time-header { display:flex; align-items:flex-start; gap:10px; margin-bottom:12px; }
  .time-icon   { font-size:22px; flex-shrink:0; }
  .time-header-text { flex:1; }
  .time-title  { font-size:12px; font-weight:bold; color:#ff6600; letter-spacing:0.5px; text-transform:uppercase; }
  .time-sub    { font-size:10px; color:#664400; margin-top:2px; }
  .time-count  { background:#2a1000; color:#ff6600; font-size:11px; font-weight:bold; padding:2px 8px; border-radius:10px; border:1px solid #ff660055; align-self:flex-start; flex-shrink:0; }
  .time-rows   { display:flex; flex-direction:column; gap:8px; margin-bottom:10px; }
  .time-row    { display:flex; gap:10px; align-items:flex-start; background:#120800; border:1px solid #2a1500; border-radius:8px; padding:10px; }
  .time-row-critical { border-left:3px solid #ff4400; }
  .time-row-warning  { border-left:3px solid #ff8800; }
  .time-row-icon  { font-size:18px; flex-shrink:0; }
  .time-row-body  { flex:1; min-width:0; }
  .time-row-title { font-size:11px; font-weight:bold; color:#ff8844; margin-bottom:3px; }
  .time-row-desc  { font-size:10px; color:#cc6622; line-height:1.4; }
  .time-row-detail { font-size:10px; color:#885533; margin-top:3px; font-style:italic; word-break:break-all; }
  .time-hint { font-size:9px; color:#663300; border-top:1px solid #2a1500; padding-top:8px; line-height:1.4; }
</style>
</head>
<body>
...
<div class="hero">
  ...
  <div class="hero-name">Dexter<span>SS</span></div>
  <div class="hero-credits">por <span class="credit-name">Dexter</span></div>
  <div class="lang-bar">
